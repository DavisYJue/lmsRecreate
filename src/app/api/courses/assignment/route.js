...
export async function GET() {
  try {
    ...
    // For each assignment, fetch both student and other submissions
    const data = await Promise.all(
      assignments.map(async (a) => {
        ...
        const allSubs = [...studentSubs, ...otherSubs];

        return {
          assignment_id: a.assignment_id,
          title: a.assignment_title,
          max_grade: a.max_grade,
          submissions: allSubs.map((s) => ({
          ...
          // optionally you could fetch not-submitted students here,
...
export async function PUT(req) {
  try {
    ...
    if (!submission_id || new_grade == null || !role || !graderId) {
      return Response.json(
    ...
    if (role !== "student") {
      return Response.json(
        { error: "Grading only supported for students." },
        { status: 400 }
      );
    }
    ...
    // Update grade & graded_by
    await query(
      `UPDATE submission
         SET grade = ?, graded_by = ?
       WHERE submission_id = ?`,
      [new_grade, graderId, submission_id]
    );
    ...
    await query(
      `INSERT INTO grade_audit
       ...
       VALUES (?, ?, ?, ?, NOW())`,
      [submission_id, old_grade, new_grade, graderId]
    );
